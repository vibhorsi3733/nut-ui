...
const GenericSidebar: React.FC<GenericSidebarProps> = ({ isOpen = false, onClose }) => {
  const pathname = usePathname();
  const [expandedComponents, setExpandedComponents] = useState<Set<string>>(
    new Set(genericComponents.map(c => c.id))
  );
  ...
  const isComponentActive = (componentId: string) => {
    return pathname?.startsWith(`/generic-components/${componentId}`);
  };

  const isVariantActive = (variantId: string, componentId: string) => {
    return pathname === `/generic-components/${componentId}/variants/${variantId}`;
  };
          ...
          <div className="mb-6">
            ...
            <nav className="space-y-1">
              {genericComponents.map((component) => {
                const componentVariants = genericVariants.filter(
                      ...
                      <div className="ml-4 mt-1 space-y-1 border-l-2 border-gray-200 dark:border-gray-800 pl-3">
                        {componentVariants.map((variant) => {
                          ...
                          return (
                            <Link
                              key={variant.id}
                              href={`/generic-components/${component.id}/variants/${variant.id}`}
                              onClick={() => {
                                if (window.innerWidth < 1024 && onClose) {
                                  onClose();
                                }
